...
cd /srv/api.kichink.com/web/
composer install --no-dev

cd /srv/api.kichink.com/web/application/config
...
if [ -d "$DIR" ]; then
  ...
  #get db creds
  aws s3 cp s3://kichink-config-dev/db/dev-data-3.0.txt . --region us-east-1
...
echo $ID $DATE >> /srv/api.kichink.com/web/content/last_updated.txt
